...
class DistributedNetwork:
    ...
    def goal_seeking_simulation(self, goal_x, goal_y):
        ...
        def animate_goal_seeking(frame):
            ...
            max_comm_range = 2.2  # Same as in update function
    ...
    def update_positions_goal_seeking(self):
        ...
        critical_edges = []
        non_critical_edges = []
        max_communication_range = 2.2  # Reduced from 2.8 for more aggressive pruning
        ...
        max_communication_range = 2.2  # Same as pruning logic
        
        for node_id, node in self.nodes.items():
            # Base Brownian motion for all robots
            brownian_force = np.random.normal(0, 0.025, 2)
            total_force = brownian_force
            
            # Goal-seeking force ONLY for closest robot
            if node_id == self.closest_robot_id:
                direction_to_goal = self.goal_location - node.position
                distance_to_goal = np.linalg.norm(direction_to_goal)
                
                if distance_to_goal > 0:
                    # Strong goal-seeking force
                    goal_force = (direction_to_goal / distance_to_goal) * 0.05
