...
def run_centralized_mlccst_simulation(underwater_env):
    ...
    if clf_distance_history:
        ...
        plt.title('CLF Convergence: Distance to Goal Over Time', fontweight='bold')
...
class DistributedNetwork:
    ...
    def goal_seeking_simulation(self, goal_x, goal_y, use_controller=True, pruning_method="triangular"):
        ...
        # Remove redundant title to avoid overlap with suptitle
        ...
        ax_details.set_title("Animation Progress", fontsize=12, fontweight='bold', pad=15)
        ...
        def animate_goal_seeking(frame):
            ...
            ax_network.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(0.99, 0.99), 
                            fontsize=9, framealpha=0.9, fancybox=True, shadow=True)
            
            # Draw statistics
            ax_stats.set_title("MST Error Analysis (Real-time)", fontsize=14, fontweight='bold', pad=15)
            if len(self.goal_distance_history) > 1:
                ...
                ax_stats.set_xlabel('Animation Steps', fontsize=11)
                ax_stats.set_ylabel('MST Error (Excess Cost)', color='red', fontsize=11, fontweight='bold')
                ax_stats.tick_params(axis='y', labelcolor='red', labelsize=10)
                ...
                ax_stats.legend(loc='upper right', fontsize=8, framealpha=0.9, fancybox=True)
            ...
            # Add MST comparison details
            if len(self.current_tree_edge_sum_history) > 0:
                current_sum = self.current_tree_edge_sum_history[-1]
                mst_sum = self.true_mst_edge_sum_history[-1]
                error = self.mst_error_history[-1]
                details_text += f"\n📈 MST ERROR ANALYSIS:\n"
                details_text += f"   Current Edge Sum: {current_sum:.1f}\n"
                details_text += f"   True MST Sum: {mst_sum:.1f}\n"
                details_text += f"   Error (Excess): {error:.2f}\n"
                
                if error <= 0.5:
                    error_emoji = "🎯 Near Optimal"
                elif error <= 2.0:
                    error_emoji = "⚠️ Moderate Error"
                else:
                    error_emoji = "🔴 High Error"
                details_text += f"   Status: {error_emoji}\n"
            
            details_text += f"\n📡 Comm. Range: 1.2 units\n"
            details_text += f"🎬 Frame: {self.goal_switch_counter}/{self.goal_switch_interval}\n"
            details_text += f"🎯 Goal: ({goal_x:.1f}, {goal_y:.1f})\n"
            
            # Add control mode information
            if use_controller:
                if pruning_method == "mlccst":
                    details_text += f"🌲 Mode: MLCCST Controller"
                elif pruning_method == "distance":
                    details_text += f"📏 Mode: Distance-based Pruning"
                else:
                    details_text += f"📐 Mode: Triangular Pruning"
            else:
                details_text += f"🎲 Mode: Pure Brownian Motion\n"
                details_text += f"    (Distance-based edges only)"
            ...
            # Status message at bottom with better formatting
            if self.goal_reached:
                status_msg = "🎯 GOAL REACHED! Simulation complete!"
                status_color = 'green'
                bg_color = 'lightgreen'
            else:
                # Create more descriptive status message
                if not use_controller:
                    algo_name = "Pure Brownian Motion + Distance-based Edges"
                    status_msg = f"🎲 {algo_name} (NO CBF control)"
                elif pruning_method == "mlccst":
                    algo_name = "MLCCST controller"
                    status_msg = f"🚀 Moving toward goal using {algo_name}..."
                elif pruning_method == "distance":
                    algo_name = "distance-based pruning"
                    status_msg = f"🚀 Moving toward goal using {algo_name}..."
                else:
                    algo_name = "triangular pruning"
                    status_msg = f"🚀 Moving toward goal using {algo_name}..."
                
                status_color = 'orange' if not use_controller else 'blue'
                bg_color = 'lightyellow'
                
            ax_details.text(0.5, 0.02, status_msg, transform=ax_details.transAxes, 
                           ha='center', va='bottom', fontsize=10, fontweight='bold',
                           color=status_color,
                           bbox=dict(boxstyle='round,pad=0.3', facecolor=bg_color, alpha=0.9, edgecolor=status_color))
        ...
        fig.suptitle("Underwater Goal-Seeking Robot Network with MST Connectivity Analysis", 
                    fontsize=16, fontweight='bold', y=0.97)
        plt.tight_layout(rect=[0, 0.03, 1, 0.93])  # Leave more space for suptitle
        plt.subplots_adjust(hspace=0.35, wspace=0.25)  # Better spacing between subplots
        plt.show()
        ...
        if self.current_tree_edge_sum_history and self.true_mst_edge_sum_history and self.mst_error_history:
            ...
            ax4.set_title('Complete Comparison', fontweight='bold', fontsize=11)
        ...
        if self.clf_distance_history:
            ...
            if self.goal_distance_history:
                ...
                plt.axhline(y=self.goal_tolerance, color='red', linestyle='--', alpha=0.7, label='Goal Threshold')
                plt.title('Closest Robot Distance to Goal', fontweight='bold')
                plt.xlabel('Animation Frame')
                plt.ylabel('Distance')
                plt.legend()
                plt.grid(True, alpha=0.3)
