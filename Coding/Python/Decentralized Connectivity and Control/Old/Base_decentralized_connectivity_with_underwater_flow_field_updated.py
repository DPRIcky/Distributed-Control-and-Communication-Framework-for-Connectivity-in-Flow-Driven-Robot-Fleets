...
def run_centralized_mlccst_simulation(underwater_env):
    ...
    if clf_distance_history:
        ...
        plt.plot(iterations, clf_distance_history, 'b-', linewidth=2, label='Average Distance to Goal')
...
class DistributedNetwork:
    ...
    def goal_seeking_simulation(self, goal_x, goal_y, use_controller=True):
        """
        Goal-seeking simulation with or without connectivity preservation
        If use_controller=True: Standard CBF-controlled goal-seeking
        If use_controller=False: Pure Brownian motion with same visualization
        """
        import matplotlib.animation as animation
        ...
        def animate_goal_seeking(frame):
            ...
            if not self.goal_reached:
                ...
                # Run decentralized edge optimization periodically (every 10 frames) - ONLY when controller is enabled
                if frame % 10 == 0 and use_controller:
                    self.run_decentralized_edge_optimization()
                elif frame % 10 == 0 and not use_controller:
                    print("   -> Skipping edge optimization (Brownian motion mode - no connectivity preservation)")
            else:
                if frame % 30 == 0:
...
def get_user_input():
    ...
    if graph_option == 1:
        ...
        if use_goal_seeking:
            ...
            if use_controller:
                pruning_method = "Goal-Seeking with CBF Control"
            else:
    ...
    return num_nodes, graph_type, description, edge_prob, graph_option, use_simultaneous_pruning, use_goal_seeking, pruning_method, use_controller
...
def main():
    ...
    while True:
        ...
        if choice in ['1', '2', '3', '4', '5', '6']:
            ...
            if choice == '6':
                # Get user input
                num_nodes, preferred_type, description, edge_prob, graph_option, use_simultaneous_pruning, use_goal_seeking, pruning_method, use_controller = get_user_input()
                ...
                if use_goal_seeking:
                    ...
                    network.goal_seeking_simulation(goal_x, goal_y, use_controller)
                elif use_simultaneous_pruning:
